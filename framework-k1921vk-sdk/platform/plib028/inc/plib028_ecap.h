...
#ifndef __PLIB028_ECAP_H
#define __PLIB028_ECAP_H
...
/**
  * @brief Выбор схемы синхронизации модулей захвата
  */
typedef enum {
    ECAP_Sync_Serial, /*!< Последовательная синхронизация всех блоков: ECAP0->ECAP1->...->ECAP5 */
    ECAP_Sync_Group,  /*!< Разбиение на группы синхронизации: ECAP0->ECAP1->ECAP2, ECAP3->ECAP4->ECAP5 */
} ECAP_Sync_TypeDef;
#define IS_ECAP_SYNC(VALUE) (((VALUE) == ECAP_Sync_Serial) || \
                             ((VALUE) == ECAP_Sync_Group))

/**
  * @brief  Выбор режима работы дополнительного входного фильтра
  */
typedef enum {
    ECAP_QualMode_3Sample, /*!< Используется 3 отсчета для фильтрации */
    ECAP_QualMode_6Sample  /*!< Используется 6 отсчетов для фильтрации */
} ECAP_QualMode_TypeDef;
#define IS_ECAP_QUAL_MODE(VALUE) (((VALUE) == ECAP_QualMode_3Sample) || \
                                  ((VALUE) == ECAP_QualMode_6Sample))
...
#define IS_ECAP_QUAL_PERIOD(VALUE) (((VALUE)&0xFFFF0000) == 0)
...
/**
  * @brief   Настройка режима дополнительного входного фильтра ECAP
  * @param   ECAPx Выбор модуля ECAP, где x лежит в диапазоне 0-5
  * @param   Mode  Выбор режима
  * @retval  void
  */
__STATIC_INLINE void ECAP_QualModeConfig(ECAP_TypeDef* ECAPx, ECAP_QualMode_TypeDef Mode)
{
    assert_param(IS_ECAP_PERIPH(ECAPx));
    assert_param(IS_ECAP_QUAL_MODE(Mode));

    if (ECAPx == ECAP0) {
        MODIFY_REG(SIU->QCQUALCTL, SIU_QCQUALCTL_CAPMODE0_Msk, Mode << SIU_QCQUALCTL_CAPMODE0_Pos);
    } else if (ECAPx == ECAP1) {
        MODIFY_REG(SIU->QCQUALCTL, SIU_QCQUALCTL_CAPMODE1_Msk, Mode << SIU_QCQUALCTL_CAPMODE1_Pos);
    } else if (ECAPx == ECAP2) {
        MODIFY_REG(SIU->QCQUALCTL, SIU_QCQUALCTL_CAPMODE2_Msk, Mode << SIU_QCQUALCTL_CAPMODE2_Pos);
    } else /*if (ECAPx == ECAP3)*/ {
        MODIFY_REG(SIU->QCQUALCTL, SIU_QCQUALCTL_CAPMODE3_Msk, Mode << SIU_QCQUALCTL_CAPMODE3_Pos);
    }
}
...
/**
  ...
  * @retval  void
  */
__STATIC_INLINE void ECAP_QualSampleConfig(ECAP_TypeDef* ECAPx, uint32_t SamplePerod)
{
    assert_param(IS_ECAP_PERIPH(ECAPx));
    assert_param(IS_ECAP_QUAL_PERIOD(SamplePerod));

    WRITE_REG(SIU->QCQUALSAMPLE_bit.CAPVAL, SamplePerod);
}

/**
  * @brief   Включение дополнительного входного фильтра ECAP
  * @param   ECAPx  Выбор модуля ECAP, где x лежит в диапазоне 0-5
  * @param   State  Выбор состояния
  * @retval  void
  */
__STATIC_INLINE void ECAP_QualCmd(ECAP_TypeDef* ECAPx, FunctionalState State)
{
    assert_param(IS_ECAP_PERIPH(ECAPx));
    assert_param(IS_FUNCTIONAL_STATE(State));

    if (ECAPx == ECAP0) {
        MODIFY_REG(SIU->QCQUALCTL, SIU_QCQUALCTL_CAPEN0_Msk, (uint32_t)State << SIU_QCQUALCTL_CAPEN0_Pos);
    } else if (ECAPx == ECAP1) {
        MODIFY_REG(SIU->QCQUALCTL, SIU_QCQUALCTL_CAPEN1_Msk, (uint32_t)State << SIU_QCQUALCTL_CAPEN1_Pos);
    } else if (ECAPx == ECAP2) {
        MODIFY_REG(SIU->QCQUALCTL, SIU_QCQUALCTL_CAPEN2_Msk, (uint32_t)State << SIU_QCQUALCTL_CAPEN2_Pos);
    } else /*if (ECAPx == ECAP3)*/ {
        MODIFY_REG(SIU->QCQUALCTL, SIU_QCQUALCTL_CAPEN3_Msk, (uint32_t)State << SIU_QCQUALCTL_CAPEN3_Pos);
    }
}

/**
  * @brief   Выбор схемы синхронизации блоков захвата
  * @param   Sync  Выбор схемы
  * @retval  void
  */
__STATIC_INLINE void ECAP_SyncConfig(ECAP_Sync_TypeDef Sync)
{
    assert_param(IS_ECAP_SYNC(Sync));

    WRITE_REG(SIU->PWMSYNC_bit.CAPSYNCSEL, (uint32_t)Sync);
}
...
/**
  ...
  * @param   ECAPx  Выбор ECAP, где x лежит в диапазоне 0-5
