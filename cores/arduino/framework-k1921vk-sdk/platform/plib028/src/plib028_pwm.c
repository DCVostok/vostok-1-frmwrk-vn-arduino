...
/**
  ...
  * @param   PWMx  Выбор модуля PWM, где x лежит в диапазоне 0-9
  * @retval  void
  */
void PWM_DeInit(PWM_TypeDef* PWMx)
{
    ...
    if (PWMx == PWM0)
        PWM_rst = RCU_APBRst1_PWM0;
    else if (PWMx == PWM1)
        PWM_rst = RCU_APBRst1_PWM1;
    else if (PWMx == PWM2)
        PWM_rst = RCU_APBRst1_PWM2;
    else if (PWMx == PWM3)
        PWM_rst = RCU_APBRst1_PWM3;
    else if (PWMx == PWM4)
        PWM_rst = RCU_APBRst1_PWM4;
    else if (PWMx == PWM5)
        PWM_rst = RCU_APBRst1_PWM5;
    else if (PWMx == PWM6)
        PWM_rst = RCU_APBRst1_PWM6;
    else if (PWMx == PWM7)
        PWM_rst = RCU_APBRst1_PWM7;
    else if (PWMx == PWM8)
        PWM_rst = RCU_APBRst1_PWM8;
    else /* if (PWMx == PWM9) */
        PWM_rst = RCU_APBRst1_PWM9;

    RCU_APBRst1Cmd(PWM_rst, DISABLE);
    RCU_APBRst1Cmd(PWM_rst, ENABLE);
}
...
void PWM_HD_Init(PWM_TypeDef* PWMx, PWM_HD_Init_TypeDef* InitStruct)
{
    ...
    PWM_HD_Source0Cmd(PWMx, InitStruct->Source0, ENABLE);
    PWM_HD_Source1Cmd(PWMx, InitStruct->Source1, ENABLE);
    PWM_HD_CycleCmd(PWMx, InitStruct->CycleEn);
    PWM_HD_OneShotCmd(PWMx, InitStruct->OneShotEn);
}
...
void PWM_HD_StructInit(PWM_HD_Init_TypeDef* InitStruct)
{
    ...
    InitStruct->Source0 = 0;
    InitStruct->Source1 = 0;
    InitStruct->CycleEn = DISABLE;
    InitStruct->OneShotEn = DISABLE;
}
...
void PWM_TZ_Init(PWM_TypeDef* PWMx, PWM_TZ_Init_TypeDef* InitStruct)
{
    ...
    PWM_TZ_CycleCmd(PWMx, InitStruct->CycleChannel, InitStruct->CycleEn);
    PWM_TZ_OneShotCmd(PWMx, InitStruct->OneShotChannel, InitStruct->OneShotEn);
}
...
void PWM_TZ_StructInit(PWM_TZ_Init_TypeDef* InitStruct)
{
    ...
    InitStruct->CycleChannel = 0;
    InitStruct->OneShotChannel = 0;
    InitStruct->CycleEn = DISABLE;
    InitStruct->OneShotEn = DISABLE;
}
