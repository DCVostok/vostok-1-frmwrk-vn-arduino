...
namespace arduino{
    ...
    void UartSerial::init_gpio_nvic_rcc(){
        ...
        GPIO_StructInit(&GPIO_InitStruct);
            ...
            if(_nt_uart == NT_UART0){
                ...
                GPIO_StructInit(&GPIO_InitStruct);
                GPIO_InitStruct.GPIO_AltFunc = pin_description_rx->port == NT_GPIOC ? GPIO_AltFunc_2 :GPIO_AltFunc_1;
                ...
                GPIO_StructInit(&GPIO_InitStruct);
                GPIO_InitStruct.GPIO_AltFunc = pin_description_tx->port == NT_GPIOC ? GPIO_AltFunc_2 :GPIO_AltFunc_1;
                GPIO_InitStruct.GPIO_Mode = GPIO_Mode_AltFunc;
            ...
            if(_nt_uart == NT_UART2){
                if(pin_description_rx->port == NT_GPIOF && pin_description_rx->pin_msk == GPIO_Pin_4){
                    NT_COMMON_REG->GPIOPCTLF_bit.PIN11 = GPIO_AltFunc_3;//turn off rx from F11
                    NT_COMMON_REG->GPIOPCTLD_bit.PIN0 = GPIO_AltFunc_3;//turn off rx from D0
                }
                if(pin_description_rx->port == NT_GPIOD && pin_description_rx->pin_msk == GPIO_Pin_0){
                    NT_COMMON_REG->GPIOPCTLF_bit.PIN11 = GPIO_AltFunc_3;//turn off rx from F11
                }
                
                
                GPIO_StructInit(&GPIO_InitStruct);
                GPIO_InitStruct.GPIO_AltFunc = pin_description_rx->port == NT_GPIOD ? GPIO_AltFunc_2 :
                                               pin_description_rx->port == NT_GPIOF && pin_description_rx->pin_msk == GPIO_Pin_4? GPIO_AltFunc_3:
                                               GPIO_AltFunc_1;
                GPIO_InitStruct.GPIO_Mode = GPIO_Mode_AltFunc;
                GPIO_InitStruct.GPIO_Pin = pin_description_tx->pin_msk;
                GPIO_InitStruct.GPIO_Dir = GPIO_Dir_In;
                ...
                GPIO_StructInit(&GPIO_InitStruct);
                GPIO_InitStruct.GPIO_AltFunc = pin_description_tx->port == NT_GPIOD ? GPIO_AltFunc_2:
            ...
            if(_nt_uart == NT_UART3){
                if(pin_description_rx->port == NT_GPIOD){
                    NT_COMMON_REG->GPIOPCTLF_bit.PIN12 = GPIO_AltFunc_3;//turn off tx from F12
                    NT_COMMON_REG->GPIOPCTLF_bit.PIN13 = GPIO_AltFunc_3;//turn off rx from F13
                }
                
                
                GPIO_StructInit(&GPIO_InitStruct);
                GPIO_InitStruct.GPIO_AltFunc = pin_description_rx->port == NT_GPIOD ? GPIO_AltFunc_2 :GPIO_AltFunc_1;
                GPIO_InitStruct.GPIO_Mode = GPIO_Mode_AltFunc;
                ...
                GPIO_StructInit(&GPIO_InitStruct);
                GPIO_InitStruct.GPIO_AltFunc = pin_description_tx->port == NT_GPIOD ? GPIO_AltFunc_2 :GPIO_AltFunc_1;
                GPIO_InitStruct.GPIO_Mode = GPIO_Mode_AltFunc;
                GPIO_InitStruct.GPIO_Dir = GPIO_Dir_Out;
                GPIO_InitStruct.GPIO_Out = GPIO_Out_En;
                GPIO_InitStruct.GPIO_Pin = pin_description_tx->pin_msk;
    ...
    void UartSerial::begin(unsigned long baudRate, uint16_t config){
            ...
            UART_DataWidth_TypeDef DataWidth = (config & SERIAL_DATA_MASK) == SERIAL_DATA_5? UART_DataWidth_5:
            ...
            UART_StopBit_TypeDef StopBit = (config & SERIAL_STOP_BIT_MASK) == SERIAL_STOP_BIT_1? UART_StopBit_1:
                                           (config & SERIAL_STOP_BIT_MASK) == SERIAL_STOP_BIT_2? UART_StopBit_2:
                                           UART_StopBit_1;

            UART_ParityBit_TypeDef ParityBit = (config & SERIAL_PARITY_MASK) == SERIAL_PARITY_EVEN? UART_ParityBit_Even:
                                                 (config & SERIAL_PARITY_MASK) == SERIAL_PARITY_ODD? UART_ParityBit_Odd:
                                                 (config & SERIAL_PARITY_MASK) == SERIAL_PARITY_MARK? UART_ParityBit_High:
                                                 (config & SERIAL_PARITY_MASK) == SERIAL_PARITY_NONE? UART_ParityBit_Disable:
                                                 (config & SERIAL_PARITY_MASK) == SERIAL_PARITY_SPACE? UART_ParityBit_Low:
