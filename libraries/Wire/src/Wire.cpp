...
void TwoWire::init_periph(void) {
  ...
  if((pin_description_sda->pin_attribute & PIN_ATTR_NEED_LS_CTRL) == PIN_ATTR_NEED_LS_CTRL){
  ...
  if((pin_description_scl->pin_attribute & PIN_ATTR_NEED_LS_CTRL) == PIN_ATTR_NEED_LS_CTRL){
    ...
    digitalWrite(adc_ls_ctrl_map[pin_description_scl->adc_ch],HIGH);
      ...
      RCC_PeriphClkCmd(_i2c == NT_I2C0 ? RCC_PeriphClk_I2C0:RCC_PeriphClk_I2C1, ENABLE);
      RCC_PeriphRstCmd(_i2c == NT_I2C0 ? RCC_PeriphRst_I2C0:RCC_PeriphRst_I2C1, ENABLE);

      GPIO_InitStruct.GPIO_AltFunc = pin_description_sda->port == NT_GPIOG ? GPIO_AltFunc_1: GPIO_AltFunc_2; //SDA0 on pin G0 use Alt1
      ...
      GPIO_InitStruct.GPIO_AltFunc = pin_description_scl->port == NT_GPIOF ? GPIO_AltFunc_1: GPIO_AltFunc_2;;//SCL0 on pin F5 use Alt1
